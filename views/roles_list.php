...
?>
        <div class='row col-md-10 justify-content-between my-5'>
            <span class='col-md-6'>
                <h1 class='h1'>Lista de roles</h1>
            </span>
            <span class='col-md-2'>
                <a href="roles_create.php" class="btn btn-primary">
        ...
        <table id="rolesTable">
            ...
            <tbody>
                <?php foreach ($rolesList as $roles): ?>
                    <tr>
                        <td class='text-center'><?= htmlspecialchars($roles['id']) ?></td>
                        <td><?= htmlspecialchars($roles['name']) ?></td>                        
                        <td><?= $roles['active'] ? 'Sí' : 'No' ?></td>
                        <td>
                            <a href="roles_edit.php?id=<?= htmlspecialchars($roles['id']) ?>" class="btn btn-warning">
                                <i class="fas fa-edit"></i> Editar
                            </a>
                            <a href="roles_delete.php?id=<?= htmlspecialchars($roles['id']) ?>" class="btn btn-danger" onclick="return confirm('¿Estás seguro de eliminar este servicio?');">
                                <i class="fas fa-trash"></i> Eliminar
                            </a>
                            <a href="permission_edit.php?id=<?= htmlspecialchars($roles['id']) ?>" class="btn btn-info">
