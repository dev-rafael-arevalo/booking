...
if (isset($_COOKIE['auth_token'])) {
    ...
    try {
        ...
        if (isset($_GET['id'])) {
            $modulesId = $_GET['id'];
            $modules = $modulesController->getId($modulesId); // Obtener servicio a editar
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($name)) {
                ...
                $modulesController->update($modulesId, $name, $active, $login);
            }
        }
...
<form method="POST" action="modules_edit.php?id=<?php echo $modulesId; ?>" class='row col-lg-10 col-md-10 justify-content-center was-validated'>
    <div class="row mb-3 col-lg-12 col-md-12">
        <label for="name" class="form-label required">Nombre del Módulo</label>
        <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($modules['name']); ?>" required>
    </div>     
    <div class="row col-lg-6 col-md-6">
        <label for="boton" class="form-label">&nbsp;</label>
        <button id='boton' name='boton' type="submit" class="btn btn-primary col-3">Actualizar Módulo</button>
    </div>
</form>
</div>
<?php
