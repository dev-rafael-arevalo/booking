    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($name) ) {
                ...
                $mensaje=$statusController->store($name, $active, $userId);
                $mensaje='Estado de la reserva creada con éxito';
...
<div class='row col-md-10 justify-content-between my-5'>
        ...
        <form method="POST" action="status_create.php" class='row col-lg-10 col-md-10 justify-content-center was-validated'>
            <div class="row mb-3 col-lg-12 col-md-12">
                <label for="name" class="form-label required">Nombre del Estado</label>
                <input type="text" class="form-control" id="name" name="name" required>
            </div>                
            <button type="submit" class="btn btn-primary col-1 mb-3">Crear Estado</button>
        </form>
    </div>    
<?php
