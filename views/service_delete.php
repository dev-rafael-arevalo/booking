...
if (isset($_COOKIE['auth_token'])) {
    ...
    try {
        ...
        if (isset($_GET['id'])) {
            $serviceId = $_GET['id'];
            $service = $serviceController->getId($serviceId); // Obtener servicio a editar
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (! isset($_GET['id'])) {
                ...
                $serviceController->delete($serviceId, $login);

                $mensaje = "Servicio ".$name." eliminado.";
...
<div class='row col-md-10 justify-content-between my-5'>
    <span class='col-md-6'>
        <h1 class='h1 text-danger'>Eliminar Servicio</h1>
...
<form method="POST" action="service_delete.php?id=<?php echo $serviceId; ?>" class='row col-lg-10 col-md-10 justify-content-center was-validated'>
    <div class="row mb-3 col-lg-12 col-md-12">
        <label for="name" class="form-label">Nombre del Servicio</label>
        <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($service['name']); ?>" readonly>
    </div>
    <div class="mb-3 col-lg-6 col-md-6">
        <label for="fa_icon" class="form-label required">Ícono (FontAwesome)</label>
        <div class="input-group">
            <input type="text" class="form-control" id="fa_icon" name="fa_icon" readonly placeholder="Selecciona un ícono" value="<?php echo htmlspecialchars($service['fa_icon']); ?>" required>
            <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#iconModal">
                <i id="selectedIcon" class="fas <?php echo htmlspecialchars($service['fa_icon']); ?>"></i> Seleccionar
            </button>
        </div>
    ...
    <div class="mb-3 col-lg-6 col-md-6">
        ...
        <select class="form-control" id="type" name="type" disabled>
            <option value="accommodation" <?php echo ($service['type'] == 'accommodation') ? 'selected' : ''; ?>>Alojamiento</option>
            <option value="room" <?php echo ($service['type'] == 'room') ? 'selected' : ''; ?>>Habitación</option>
        </select>
    </div>    
    <div class="row col-lg-6 col-md-6">
        <label for="boton" class="form-label">&nbsp;</label>
        <button id='boton' name='boton' type="submit" class="btn btn-danger col-3">Eliminar Servicio</button>
    </div>
</form>
